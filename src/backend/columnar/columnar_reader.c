				  ...
				  bool snapshotCreatedByUs)
{
	...
	readState->snapshotCreatedByUs = snapshotCreatedByUs;
...
{
	if (readState->snapshotCreatedByUs)
	{
		/*
		 * init_columnar_read_state created a new snapshot and pushed it as
		 * the active one, so now forget it.
		 */
		PopActiveSnapshot();
	}
