...
BEGIN
    ...
    LOOP
        ...
        -- its shards will be dropped when dropping the parent's shards,
        -- so we can skip: i.e. we call citus_drop_all_shards with drop_shards_metadata_only parameteter set to true
        ...
        THEN
            PERFORM citus_drop_all_shards(v_obj.objid, v_obj.schema_name, v_obj.object_name, true);
        ELSE
            PERFORM citus_drop_all_shards(v_obj.objid, v_obj.schema_name, v_obj.object_name, false);
        END IF;
